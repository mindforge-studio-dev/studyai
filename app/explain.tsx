...
export default function ExplainScreen() {
  ...
  const handleExplain = async () => {
    ...
    startTracking();
    setGenerating(true); setResult(null); setSaved(false);
    try {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newEntry: CachedExplanation = {
        ...
        inputText: result.inputText, explanation: result.explanation,
        keyPoints: result.keyPoints, difficulty: result.difficulty, createdAt: result.createdAt,
      };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F8F9FA" }}>
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 60 }} keyboardShouldPersistTaps="handled">
          ...
          {(["facile", "moyen", "difficile"] as const).map((d) => (
            <TouchableOpacity
              ...
              style={{
                ...
                borderWidth: 1, borderColor: difficulty === d ? difficultyColors[d] : "#E5E7EB",
              }}
            >
              <Text style={{ fontWeight: "600", fontSize: 13, color: difficulty === d ? "#FFFFFF" : "#374151" }}>
                {difficultyLabels[d]}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <TouchableOpacity
          ...
          style={{ backgroundColor: generating ? "#A5B4FC" : "#6366F1", borderRadius: 14, padding: 16, alignItems: "center", elevation: 4, marginBottom: 24 }}
        >
          {generating ? (
        ...
        {result && (
          <View>
            ...
            <View style={{
              ...
              borderLeftColor: "#6366F1", borderRightColor: "#6366F1",
            }}>
              <Text style={{ fontSize: 14, fontWeight: "700", color: "#3730A3", marginBottom: 8, textAlign: isRTL ? "right" : "left" }}>
                💡 {t("explanation")}
              </Text>
              <Text style={{ fontSize: 14, color: "#3730A3", lineHeight: 22, textAlign: isRTL ? "right" : "left", writingDirection: isRTL ? "rtl" : "ltr" }}>
                {result.explanation}
              </Text>
            </View>
            ...
            {result.keyPoints.length > 0 && (
              <View style={{ backgroundColor: "#FFFFFF", borderRadius: 14, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: "#E5E7EB" }}>
                <Text style={{ fontSize: 14, fontWeight: "700", color: "#111827", marginBottom: 10, textAlign: isRTL ? "right" : "left" }}>
                  🔑 {t("key_points")}
                </Text>
                {result.keyPoints.map((point, i) => (
                  <View key={i} style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "flex-start", marginBottom: 6 }}>
                    <View style={{
                      ...
                      <Text style={{ fontSize: 11, fontWeight: "700", color: "#6366F1" }}>{i + 1}</Text>
                    </View>
                    <Text style={{ fontSize: 13, color: "#374151", flex: 1, lineHeight: 20, textAlign: isRTL ? "right" : "left", writingDirection: isRTL ? "rtl" : "ltr" }}>
                      {point}
                    </Text>
                  </View>
                ))}
        ...
        {cachedExplanations.length > 0 && !result && (
          <View style={{ marginTop: 24 }}>
            ...
            {cachedExplanations.slice(0, displayCount).map((item) => (
              ...
              >
                <Text style={{ fontSize: 13, color: "#374151", lineHeight: 18, textAlign: isRTL ? "right" : "left" } as any} numberOfLines={2}>
                  {item.explanation}
                </Text>
